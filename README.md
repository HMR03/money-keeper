...
每次修改代码并推送 GitHub 前，**必须同步修改两处版本号**：

```
1. sw.js        → CACHE_NAME  末尾数字 +1
2. index.html   → APP_VERSION 末尾数字 +1  （搜索 "APP_VERSION"）
```

两处版本号必须一致，否则设置页显示的版本号和实际缓存不一致。

**更新机制**：SW 自动激活、缓存新文件、页面自动刷新。用户打开 App 即是最新版，底部短暂显示「✨ 已更新到最新版本」。
